...
char* serializeMetaData(RecordManager recordManager, Schema *schema)
{
    VarString *result;
    MAKE_VARSTRING(result);
    APPEND(result,"tableName:%s",recordManager.tableData->name);
    APPEND(result, " FreePage:%i FreeSlot:%i\n",recordManager.freePageManager->freePage,recordManager.freePageManager->freePage);


    APPEND(result, "SchemaAttributes:%i ", schema->numAttr);
...
serializeRecord(Record *record, Schema *schema)
{
	...
	for(i = 0; i < schema->numAttr; i++)
	{
        char* str = serializeAttr (record, schema, i);
		APPEND_STRING(result, str);
		APPEND(result, "%s", (i == 0) ? "" : ",");
        free(str);
	}
