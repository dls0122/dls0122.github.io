...
testRecords (void)
{
    ...
    Value * v1 = stringToValue("i1");
	OP_TRUE(v1, value, valueEquals, "first attr");
    freeVal(v1);
	freeVal(value);

	getAttr(r, schema, 1, &value);
    Value * v2 = stringToValue("saaaa");
	OP_TRUE(v2, value, valueEquals, "second attr");
	freeVal(value);
    freeVal(v2);

	getAttr(r, schema, 2, &value);
    Value * v3 = stringToValue("i3");
	OP_TRUE(v3, value, valueEquals, "third attr");
	freeVal(value);
    free(v3);
    Value * v4 = stringToValue("i4");
	//modify attrs
	setAttr(r, schema, 2, v4);
	getAttr(r, schema, 2, &value);

	OP_TRUE(v4, value, valueEquals, "third attr after setting");
	freeVal(value);
    freeVal(v4);
...
testCreateTableAndInsert (void)
{
	...
	{   TEST_CHECK(createRecord(&r, schema));  // added Fall 2021
        ...
        Record * r1 = fromTestRecord(schema, inserts[pos]);
...
testUpdateTable (void)
{
	...
	for(i = 0; i < numFinal; i++)
	{
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid, r));
        Record * r1 = fromTestRecord(schema, finalR[i]);
		ASSERT_EQUALS_RECORDS(r1, r, schema, "compare records");
        freeRecord(r1);
	}
...
testInsertManyRecords(void)
{
	...
	for(i = 0; i < numInserts; i++)
	{
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid, r));
        Record* r1 = fromTestRecord(schema, realInserts[i]);
		ASSERT_EQUALS_RECORDS(r1, r, schema, "compare records");
        freeRecord(r1);
	}
    freeRecord(r);   // Added Summer 2021
    ...
    Record * r1 = fromTestRecord(schema, updates[0]);
	ASSERT_EQUALS_RECORDS(r1, r, schema, "compare records");
...
void testScansTwo (void)
{
	...
	while((rc = next(sc, r)) == RC_OK)
	{
        Record * r1 = fromTestRecord(schema, inserts[1]);
		ASSERT_EQUALS_RECORDS(r1, r, schema, "compare records");
        freeRecord(r1);
	}
    freeRecord(r);
	...
	while((rc = next(sc, r)) == RC_OK)
	{
        Record * r1= fromTestRecord(schema, inserts[5]);
		ASSERT_EQUALS_RECORDS(r1, r, schema, "compare records");
...
//    freeExpr(left);
	...
	while((rc = next(sc, r)) == RC_OK)
	{
		...
		for(i = 0; i < numInserts; i++)
		{
            Record * r1 = fromTestRecord(schema, inserts[i]);
			if (memcmp(r1->data,r->data,getRecordSize(schema)) == 0)
				foundScan[i] = TRUE;
            freeRecord(r1);
		}
